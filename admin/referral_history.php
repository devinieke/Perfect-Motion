                ...
                <div class="page-content">
                    ...
                    <div class="page-head">
                        ...
                        <div class="page-title">
                            <h1>List of Requested Referral Payouts
                            </h1>
                        </div>
                        <!-- END PAGE TITLE -->
                        <!-- BEGIN PAGE TOOLBAR -->
                                        ...
                                        <table class="table table-striped table-bordered table-hover table-checkable">
                                            <thead>
                                                ...
                                                <?php
                                                    ...
                                                    $check = "SELECT * FROM ref_payout_request WHERE payout_status=1";
                                                    $checkResult = mysqli_query($connection, $check) or die(mysqli($connection));

                                                    while($checkRow = mysqli_fetch_array($checkResult))
                                                    {
                                                        $checkid = $checkRow['ref_ID'];
                                                        $checkDate = $checkRow['Created'];

                                                        $refTotal = "SELECT sum(Amount) FROM ref_payout_request WHERE payout_status=1 AND ref_ID='$checkid'";
                                                        $refTotalResult = mysqli_query($connection, $refTotal) or die(mysqli_error($connection));

                                                        while($refRow = mysqli_fetch_array($refTotalResult))
                                                        {
                                                            $refAmount = $refRow['sum(Amount)'];   

                                                            $entryCheck = "SELECT * FROM user WHERE User_ID='$checkid'";
                                                            ...
                                                            while($entryCheckRow = mysqli_fetch_array($entryCheckResult))
                                                            {
                                                                ...
                                                                $entryBankName = $entryCheckRow['Bank_Name'];
                                                                $entryAccNo = $entryCheckRow['Acc_No'];
                                                    ...
                                                    <!-- <td>
                                                        <div class="margin-bottom-5">
                                                            <form action="" method="post">
                                                                <input type="hidden" name="confirmRefID" value="<?php //echo $entryUnique?>" class="form-control">
                                                                <button class="btn btn-sm green btn-outline filter-submit margin-bottom" name="confirmRef" type="submit">
                                                                    <i class="fa fa-check"></i> Confirm Referral
                                                                </button>
                                                            </form>
                                                        </div>
